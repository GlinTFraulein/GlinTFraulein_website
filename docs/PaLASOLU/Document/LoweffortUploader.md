...
Avatar Menuの仕様については、`Tools/PaLASOLU/Extensions/PaLASOLU Avatar Menu (World Fixed)`からご確認ください。Low-effot UploaderはBuild時にWorldFixedの子に再設定されます。

`PaLASOLU_Play_Trigger`というパラメータ名を使用することができます。これは「ライブ再生」時にONになる、**自動リネームされない**パラメーターです。

アバター側に`PaLASOLU_Play_Trigger`パラメータで動くギミックを仕込むことで、パーティクルライブの発火タイミングとギミックの動作タイミングを同期させることができます。

### Disable on Play Object
本設定は、`Generate Avatar Menu`がONの場合にのみ存在するオプションです。

`Generate Avatar Menu`で生成されたExpression Menuで、「ギミックON」時に表示され、「ライブ再生」時に非表示になるようなGameObjectを設定します。

何も設定されていない場合は、パーティクルライブ視聴目安位置を示す白線(PaLASOLU棒と呼ばれています)が表示されます。

つまり、このオプションは「PaLASOLU棒を置き換える」ことを目的にしたオプションです。

### Local Only Object
本設定は、`Generate Avatar Menu`がONの場合にのみ存在するオプションです。

`Generate Avatar Menu`で生成されたExpression Menuで、「ギミックON」「ライブ再生」時に自分のみに表示されるGameObjectを設定します。

いわゆる"バミリ"などを導入することを目的にしたオプションです。パーティクルライブと同様にワールド固定されます。
...
`PaLASOLU 2.4.0`現在、そのような場合の動作挙動は確認していません。
:::
...
### Fix Allow Roll
Particle SystemのAllow RollをOFFに固定します。

Allow RollがONの状態では、VR環境で(円形でない)Particle Systemを視界に入れた状態で首を傾けると、HMD方向に応じてパーティクル粒子が回転するようになります。これは視覚的な違和感をもたらすことがよくあります。([Unityドキュメント](https://docs.unity3d.com/ja/2018.4/Manual/PartSysRendererModule.html)でも言及されています。)



## 制約
PaLASOLU v2.4.0時点では、以下の制約があります。いろいろ書いてありますが、ほとんどのユースケースでは気にする必要がないはずです。
